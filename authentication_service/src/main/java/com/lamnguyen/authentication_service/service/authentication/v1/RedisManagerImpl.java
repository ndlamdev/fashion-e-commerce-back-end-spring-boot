...
public class RedisManagerImpl implements IRedisManager {
    ...
    public void setRegisterCode(long userId, String code) {
        setCodeVerify(userId, code, accountVerification.getKey() + ":CODE", accountVerification.getExpire(), accountVerification.getTotalTryKey(), accountVerification.getTotalResendKey(), accountVerification.getTotalResendExpire());
    }

    @Override
    public Optional<String> getRegisterCode(long userId) {
        return getValue(accountVerification.getKey() + ":CODE", userId);
    }

    @Override
    public void removeVerifyRegisterCode(long userId) {
        redisTemplate.delete(generateKey(accountVerification.getKey() + ":CODE", userId));
    ...
    public void setResetPasswordCode(long userId, String code) {
        setCodeVerify(userId, code, resetPasswordVerification.getKey() + ":CODE", resetPasswordVerification.getExpire(), resetPasswordVerification.getTotalTryKey(), resetPasswordVerification.getTotalResendKey(), resetPasswordVerification.getTotalResendExpire());
    }

    @Override
    public Optional<String> getResetPasswordCode(long userId) {
        return getValue(resetPasswordVerification.getKey() + ":CODE", userId);
    }

    @Override
    public void removeResetPasswordCode(long userId) {
        redisTemplate.delete(generateKey(resetPasswordVerification.getKey() + ":CODE", userId));
    ...
    public void setRegisterTokenIdUsingGoogle(String id) {
        redisTemplate.opsForValue().set(applicationProperty.getKeyRegisterTokenUsingGoogle() + id, 1, applicationProperty.getExpireRegisterToken(), TimeUnit.MINUTES);
    }
    ...
    public void setRegisterTokenIdUsingFacebook(String id) {
        redisTemplate.opsForValue().set(applicationProperty.getKeyRegisterTokenUsingFacebook() + id, 1, applicationProperty.getExpireRegisterToken(), TimeUnit.MINUTES);
    }
    ...
    public void setAccessTokenFacebook(String token) {
        redisTemplate.opsForValue().set(applicationProperty.getKeyAccessTokenFacebook() + token, 1, applicationProperty.getExpireRegisterToken() * 5L, TimeUnit.MINUTES);
    }
