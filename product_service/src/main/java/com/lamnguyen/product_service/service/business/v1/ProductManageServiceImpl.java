...
public class ProductManageServiceImpl implements IProductManageService {
    ...
    ProductDtoRedisManagerImpl productDtoRedisManagerImpl;
    IVariantGrpcClient variantGrpcClient;
    ...
    public List<AdminSubProductResponse> getAll() {
        var products = productMapper.toAdminSubProductResponse(productRepository.findAll());
        var ids = products.stream().map(AdminSubProductResponse::getId).toList();
        var mapInfo = variantGrpcClient.getVariantAndInventoryInfo(ids);
        products.forEach(product -> {
            var info = mapInfo.getOrDefault(product.getId(), null);
            if (info == null) return;
            product.setTotalVariants(info.getTotalVariants());
            product.setTotalInventories(info.getTotalInventories());
        });
        return products;
    }
}
